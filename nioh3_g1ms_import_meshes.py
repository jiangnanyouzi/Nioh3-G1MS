# Mesh importer for Nioh 3 G1M files.
#
# Based on g1m_import_meshes.py by GitHub/eArmada8/gust_stuff
# Adapted for Nioh 3 (G1MG 0x30303435) which adds meshlet sections.
#
# This code depends on g1m_export_meshes.py, lib_fmtibvb.py and the pyffi_tstrip module
# being in the same folder.
#
# Steps:
# 1. Run nioh3_g1m_export_meshes.py
        ...
        if len(existing_submeshes) > 0:
            try:
                ...
                for j in range(len(existing_submeshes)):
                    ...
                    # Topology change detection for Nioh 3 meshlet models
                    # In Nioh 3, meshlet submeshes often have tiny indexCount in SUBMESH metadata
                    # because the real indices live in meshlet sections (0x1000B/0x1000C).
                    # We detect meshlet usage by checking if the submesh indexCount is very small
                    # while the exported IB has many triangles.
                    orig_sub = subvbs['data'][existing_submeshes[j]]
                    orig_count = orig_sub.get('indexCount', 0)
                    new_tri_count = len(ib)
                    if orig_count < 10 and new_tri_count > 10:
                        print("WARNING: Submesh {0} appears to be meshlet-driven (orig indexCount={1}, exported tris={2}).".format(
                            existing_submeshes[j], orig_count, new_tri_count))
                        print("         If you modified topology, meshlet sections will be stale.")
                        print("         Use --strip-meshlets if the model renders incorrectly in-game.")
...
# Meshlet section magics that should be stripped when --strip-meshlets is used
MESHLET_MAGICS = {
    0x0001000a, 0x0001000b, 0x0001000c, 0x0001000d,
    0x0001000e, 0x0001000f, 0x00010010, 0x00010011,
    0x00010012, 0x00010013, 0x00010014, 0x00010015,
    0x00010016,
}

def build_g1mg(g1m_name, skel_data, e = '<', strip_meshlets = False):
    ...
    # Nioh 3 uses unknown meshlet sections (0x1000A-0x10016) whose offsets and
    # internal pointers depend on the exact layout of the preceding known sections.
    # When we rebuild VERTEX_BUFFERS/INDEX_BUFFER their sizes change, so the
    # original offsets become invalid.  The only safe option is to drop them.
    # Users who only edit vertex attributes can try --keep-meshlets at their own risk.
    if strip_meshlets:
        original_count = len(model_mesh_metadata['sections'])
        model_mesh_metadata['sections'] = [s for s in model_mesh_metadata['sections'] if s['magic'] not in MESHLET_MAGICS]
        removed = original_count - len(model_mesh_metadata['sections'])
        if removed:
            print("Stripped {} meshlet section(s).".format(removed))
...
def build_g1m(g1m_name, strip_meshlets = False):
    if os.path.exists(g1m_name) and (os.path.isdir(g1m_name)):
        with open(g1m_name + '.g1m', "rb") as f:
            ...
            new_g1mg_data = build_g1mg(g1m_name, model_skel_data, e, strip_meshlets=strip_meshlets)
...
def process_g1m(g1m_name, strip_meshlets = True):
    new_g1m_data = build_g1m(g1m_name, strip_meshlets=strip_meshlets)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        import argparse
        parser = argparse.ArgumentParser(description='Nioh 3 G1M mesh importer')
        parser.add_argument('--keep-meshlets', help="Keep meshlet sections (DANGEROUS: offsets will be stale if any buffer size changed)",
                            action="store_true")
        ...
        if os.path.exists(args.g1m_filename) and args.g1m_filename[-4:].lower() == '.g1m' and os.path.isdir(args.g1m_filename[:-4]):
            process_g1m(args.g1m_filename[:-4], strip_meshlets=not args.keep_meshlets)
    else:
